...
deploy-web:
	@if [ "$(DEPLOY_WEB_APPLY)" = "1" ]; then \
		...
		env WEB_API_BASE_URL="$(WEB_API_BASE_URL)" node -e 'const net = require("net"); const raw = process.env.WEB_API_BASE_URL || ""; let u; try { u = new URL(raw); } catch (e) { console.error("WEB_API_BASE_URL must be an absolute URL for DEPLOY_WEB_APPLY=1."); process.exit(2); } const host = u.hostname.toLowerCase(); const ip = host.startsWith("[") ? host.slice(1, -1) : host; const fam = net.isIP(ip); const blocked = new net.BlockList(); blocked.addSubnet("127.0.0.0", 8, "ipv4"); blocked.addAddress("::1", "ipv6"); blocked.addSubnet("::ffff:127.0.0.0", 104, "ipv6"); const name = host.replace(/\.$$/, ""); if (name === "localhost" || name.endsWith(".localhost") || (fam !== 0 && blocked.check(ip, fam === 4 ? "ipv4" : "ipv6"))) { console.error("WEB_API_BASE_URL must be set to the deployed tunnel origin for DEPLOY_WEB_APPLY=1."); process.exit(2); } if (u.protocol !== "https:") { console.error("WEB_API_BASE_URL must use https for DEPLOY_WEB_APPLY=1."); process.exit(2); } const normalized = raw.trim().replace(/\/+$$/, ""); if (normalized !== u.origin) { console.error("WEB_API_BASE_URL must be an exact https origin (no path, query, fragment, credentials, or default port) for DEPLOY_WEB_APPLY=1."); process.exit(2); }' || exit $$?; \
	fi
	$(MAKE) web-build
	@if [ "$(DEPLOY_WEB_APPLY)" != "1" ]; then \
